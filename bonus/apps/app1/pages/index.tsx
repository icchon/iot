...
export default function Home() {
  ...
  return (
    <div className="container">
      <Head>
        <title>App 1: 犬派 vs 猫派</title>
      </Head>

      <main>
        <div className="app-id-badge">APP 1</div>
        <h1>どっち派？ (犬 vs 猫)</h1>
        <div className="voting-section">
          <button onClick={() => vote('dog')} disabled={loading} className="btn dog">🐶 犬派</button>
          <button onClick={() => vote('cat')} disabled={loading} className="btn cat">🐱 猫派</button>
        </div>

        <div className="results">
          <h2>現在の集計</h2>
          <div className="stat">
            <span>犬: {results.dog || 0} 票</span>
            <span>猫: {results.cat || 0} 票</span>
          </div>
        </div>
        <div className="version">v0.3.0</div>
      </main>

      <style jsx>{`
        .container { min-height: 100vh; display: flex; flex-direction: column; align-items: center; justify-content: center; font-family: sans-serif; background-color: #f0f4f8; }
        ...
        .dog { background-color: #3b82f6; color: white; }
        .cat { background-color: #ef4444; color: white; }
