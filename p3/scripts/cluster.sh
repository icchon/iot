...
echo "--- Exposing Argo CD via Ingress ---"
kubectl apply -f conf/argocd/argocd-server-config.yaml
kubectl apply -f conf/argocd/argocd-ingress.yaml

echo "--- Waiting for Argo CD to be ready ---"
kubectl wait --for=condition=Available deployment/argocd-server -n argocd --timeout=300s

echo "--- Cluster is Ready! ---"
echo "Access Argo CD at: http://localhost:8888/"
